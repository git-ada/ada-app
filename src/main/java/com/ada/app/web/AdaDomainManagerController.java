...
@Controller
@Module(name="域名")
@RequestMapping(value = "/ada-domain")
public class AdaDomainManagerController extends AbstractJQueryEntityController<AdaDomain, AdaDomainService> {
	/**
	 * 权限列表
	 */
	...
	@Autowired
	private AdaDomainService adaDomainService;
	...
	@ResponseBody
	public JsonEntityResult<AdaDomain> get(HttpServletRequest request,HttpServletResponse response, Model model) {
	...
	@ResponseBody
	public JsonEntityResult<AdaDomain> save(HttpServletRequest request,HttpServletResponse response) {
	...
	@ResponseBody
	public JsonEntityResult<AdaDomain> update(HttpServletRequest request,HttpServletResponse response) {
	...
	protected String[] getExportTitles() {
		return new String[]{"链接ID","站点ID","网页链接地址","创建时间",""};
	}
}
