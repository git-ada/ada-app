...
public interface AdaDomainNotAd15mStatDao extends EntityJpaDao<AdaDomainNotad15mStat, Integer> {
	 ...
	 * 通过域名ID查询并根据开始时间排序
	 */
	@Query(value = "select * from ada_domain_notad_15m_stat obj where obj.domainId=?  order by obj.endTime desc limit 0,?",nativeQuery=true)
	public List<AdaDomainNotad15mStat> findByDomainIdOrderByStartTime(Integer domainId,int pageSize);
