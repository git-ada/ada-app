...
public interface AdaDomainAd15mStatDao extends EntityJpaDao<AdaDomainAd15mStat, Integer> {
	 ...
	 * 通过域名ID查询并根据开始时间排序
	 */
	@Query(value = "select * from ada_domain_ad_15m_stat obj where obj.domainId=?  order by obj.endTime desc limit 0,? ",nativeQuery=true)
	public List<AdaDomainAd15mStat> findByDomainIdOrderByStartTime(Integer domainId,int pageSize);
