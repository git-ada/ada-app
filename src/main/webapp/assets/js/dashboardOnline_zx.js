...
//				    var sumM = item[19]+item[20]+item[21]+item[22];
				    ...
				    **/
				    ...
				    if(Percentagemin(sumM,IP)<1){
				    	sumMTd = "<td class='onlytd' title='"+Percentage(sumM,IP)+"'><span class='tdonly'>"+fromInt(sumM)+"</span></td>";
				    }else{
...
function openMenu(a,event){
	...
	var backUrl = webPath+"/dashboardOnlineHistry.jhtm?dataType="+dataType+"&clickDate="+clickDate+"&top="+ipTop+"&doamin="+search;
	var id = jQuery(a).attr("id");
	clickChartDomainId = id;
	var domain = jQuery(a).attr("domain");

	jQuery("#onlyOne").attr("onclick","onlyOne('"+domain+"')");
	jQuery("#oneTime").attr("onclick","gotoPage('"+webPath+"/domainTimechartList_oneOnline.jhtm?domainId="+id+"&dataType="+dataType+"&domain="+domain+"&search_date="+clickDate+"','"+backUrl+"')");
	jQuery("#AdVSNotAd").attr("onclick","gotoPage('"+webPath+"/dashboard_domainTimeOnline.jhtm?domainId="+id+"&domain="+domain+"&dataType="+dataType+"&search_date="+clickDate+"','"+backUrl+"')");
