	...
	<div class="portlet-body">
		<div class="row">
		    ...
		    <div class="col-md-6 col-sm-12">
		       <button type="button" class="btn btn-default" onclick="gotoPage('${pageContext.request.contextPath}/ada-channel-link/create.jhtm?channelId=${param.search_EQ_channelId}')" data-opt-key="/ada-channel-link/create"><i class="fa fa-plus"></i>&nbsp;&nbsp;添加&nbsp;&nbsp;</button>
		       <button type="button" class="btn btn-default"  onclick="gotoHistoryPage(-1);"><i class="fa fa-canclel"></i>&nbsp;&nbsp;返回&nbsp;&nbsp;</button>
		    </div>
		    <!-- 左上角功能区 END -->
		    <!-- 头部分页 END--> 
		</div>
	            ...
	            <tbody>
	               <c:forEach var="item" items="${page.pageResults}" varStatus="number">
	                <tr>      
	                    ...
	                    <td>
	                    	<a class="btn btn-default btn-outline btn-sm opt-edit" data-id="${item.id}"  data-opt-key="/ada-channel-link/create"><span>&nbsp;&nbsp;编辑&nbsp;&nbsp;</span></a>
	                    	<a class="btn btn-default btn-outline btn-sm opt-delete" data-id="${item.id}"  data-opt-key="/ada-channel-link/delete"><span>&nbsp;&nbsp;删除&nbsp;&nbsp;</span></a>
	                    </td>
	                </tr>
	                </c:forEach>
...
<script type="text/javascript">
	...
	$(".opt-export").click(function() { 
		//refreshPage();
		var exportUrl = "${pageContext.request.contextPath}/ada-channel-link/export.do?"+$("#search_from").find("[value!='']").serialize();
    ...
    $(".opt-batch-delete").confirmation({
     ...
     var deleteItems = function(ids){
    	 jQuery.ajax({
      		url:"${pageContext.request.contextPath}/ada-channel-link/delete.do?id="+ids,
        ...
        <div class="tab-pane page-quick-sidebar-settings">
            <div class="page-quick-sidebar-settings-list">
                ...
                <div class="list-items">
					<form id="edit_form" action="${pageContext.request.contextPath}/ada-channel-link/update.do" class="form-horizontal" method="post">
			        	<input type="hidden" name="id">
		               	<div class="form-body">
			               	<div class="form-group">
			               		<label class="col-md-4 control-label">网页链接地址<span class="required" > * </span></label>	
			               		<div class="col-md-8">
					                <input type="text" name="url" class="form-control" required="required" maxlength="64">
								</div>
			               	</div>
			               	<div class="form-group">
			               		<label class="col-md-4 control-label">参数</label>	
			               		<div class="col-md-8">
					                <input type="text" name="parameter" class="form-control"  maxlength="64">
								</div>
			               	</div>
		               	</div>  
			        </form> 
...
<script type="text/javascript">
    ...
    $('.opt-edit').click(function(){
    	 var id = $(this).attr("data-id");
    		 $.ajax({  
                 url : "${pageContext.request.contextPath}/ada-channel-link/get.do",
                 data : {"id" : id},
                 success : function(ret) {
                 		...
                 		$("#edit_form").find("[name='parameter']").val(ret.entity.parameter);
    ...
    $(".opt-edit-delete").confirmation({
        ...
        onConfirm: function(){
        	...
        	jQuery.ajax({
          		url:"${pageContext.request.contextPath}/ada-channel-link/delete.do?id="+id,
          		success:function(ret){
          			if(ret.success){
          				$("body").toggleClass("page-quick-sidebar-open");
	           			toastr.success(ret.message);
