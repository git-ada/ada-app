	        ...
	        <table class="table table-striped dataTableg table-bordered table-hover data-table">
	            <thead>
	                <tr>
	                    ...
	                    <!-- 
						<th scope="col" style="width: 100px">渠道ID</th>
						 -->		
						<th scope="col" style="width: 350px">渠道名称</th>
				        <th scope="col">操作</th>
				     </tr>
	            </thead>
	            <tbody>
	               <c:forEach var="item" items="${page.pageResults}" varStatus="number">
	                <tr>      
	                    ...
	                    <td>
	                    	<a class="btn btn-default btn-outline btn-sm opt-manage-link" data-id="${item.id}"  ><span>&nbsp;&nbsp;管理渠道入口链接&nbsp;&nbsp;</span></a>
	                    	<a class="btn btn-default btn-outline btn-sm opt-edit" data-id="${item.id}"  data-opt-key="/ada-channel/create"><span>&nbsp;&nbsp;编辑&nbsp;&nbsp;</span></a>
	                    	<a class="btn btn-default btn-outline btn-sm opt-delete" data-id="${item.id}"  data-opt-key="/ada-channel/delete"><span>&nbsp;&nbsp;删除&nbsp;&nbsp;</span></a>
	                    </td>
	                </tr>
	                </c:forEach>
...
<script type="text/javascript">
	...
	$(".opt-export").click(function() { 
		//refreshPage();
		var exportUrl = "${pageContext.request.contextPath}/ada-channel/export.do?"+$("#search_from").find("[value!='']").serialize();
    ...
    $(".opt-batch-delete").confirmation({
     ...
     var deleteItems = function(ids){
    	 jQuery.ajax({
      		url:"${pageContext.request.contextPath}/ada-channel/delete.do?id="+ids,
     ...
     var disableOpts = function(){
   	...
   	$(".opt-manage-link").click(function(){
      	    var id = $(this).attr("data-id");
      		var url =  "${pageContext.request.contextPath}/ada-channel-link/list.jhtm?search_EQ_channelId="+id;
            gotoPage(url);
    });
</script>
        ...
        <div class="tab-pane page-quick-sidebar-settings">
            <div class="page-quick-sidebar-settings-list">
                ...
                <div class="list-items">
					<form id="edit_form" action="${pageContext.request.contextPath}/ada-channel/update.do" class="form-horizontal" method="post">
			        	<input type="hidden" name="id">
		               	<div class="form-body">
			               	<div class="form-group">
			               		<label class="col-md-4 control-label">渠道名称<span class="required" > * </span></label>	
			               		<div class="col-md-8">
					                <input type="text" name="channelName" class="form-control" required="required" maxlength="64">
								</div>
			               	</div>
		               	</div>  
			        </form> 
...
<script type="text/javascript">
    ...
    $('.opt-edit').click(function(){
    	 var id = $(this).attr("data-id");
    		 $.ajax({  
                 url : "${pageContext.request.contextPath}/ada-channel/get.do",
                 data : {"id" : id},
                 success : function(ret) {
                 		$("#edit_form").find("[name='id']").val(ret.entity.id);
                 		$("#edit_form").find("[name='siteId']").val(ret.entity.siteId);
                 		$("#edit_form").find("[name='channelName']").val(ret.entity.channelName);
    ...
    $(".opt-edit-delete").confirmation({
        ...
        onConfirm: function(){
        	...
        	jQuery.ajax({
          		url:"${pageContext.request.contextPath}/ada-channel/delete.do?id="+id,
          		success:function(ret){
          			if(ret.success){
          				$("body").toggleClass("page-quick-sidebar-open");
	           			toastr.success(ret.message);
